...
2. **δ-scaling itself ships regardless of which constant wins — now as the real production path, not a query-time estimate.** `computeInsertionDelta()` in `pamiStore()` is the mechanism that actually runs: O(1) at query time, a proven non-overlap guarantee (exact by default; a bounded ≤2× version if a caller opts into `useVesselPrecision`/`volatility`), self-correcting on every write. `adaptiveDelta()` (query-time density) remains a real, tested, useful function too — it's what the scorecard's Domain 1b analysis and `phi-minimax-scorecard.ts` use to reason about a population as a whole — but it is no longer the retrieval-time path; that's insertion-time δ now. Neither is contingent on which of φ/e/√2/π ever becomes `scaleBase`, and all of it is already merged, not proposed.
3. **The harness for the real experiment — genuinely not yet buildable, and here's the actual blocker.** Every number here still comes from `memorySignalNeutral()` — a synthetic, linearly-spaced signal generator. `epsMin`, `epsMinAdaptive`, `peakDiscrepancy`, and `recoveryHorizon` are written to take any `base` and any residual signal, which reads like "swap the data source and you're done" — it isn't. PAMI's `pamiIndex()` takes a numeric residual signal (`number[]`); real stored memories in `elle_memory`/`consolidate.ts` are text (`content`/`summary`). Turning real memory content into the kind of numeric residual window PAMI's wavelet transform actually operates on is an **undesigned step** — there's no existing text-to-signal projection in this codebase to reuse, and inventing an arbitrary one (character frequencies, token embeddings truncated to a window, whatever) just to have "real data" flowing through would be exactly the kind of fabricated rigor this whole line of work has been arguing against. This is a real, open design question — what does a "residual" even mean for a text memory — not a data-source swap, and it should be scoped and decided on its own before anyone points this scorecard at production content.
4. **A design-time reference, not an automated gate.** Until (3) has an actual answer, this scorecard is something to *consult*, not something to wire into a build gate or a runtime default. Given three different legitimate constructions already produce three different winners, treating any one of them as license to hardcode a constant would be exactly the kind of premature collapse the rest of this line of work has been arguing against. The honest use today: when someone proposes changing `scaleBase` or `GOLDEN_WINDING`, run this suite, read the static, adaptive, AND accuracy framings side by side, and make the call with all three numbers on the table — not just the one that was convenient.
