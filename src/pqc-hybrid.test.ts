...
import { importVector, exportVector, toHex, fromHex, type PqcVector } from './pqc-interop';
import laptopVectors from '../test-fixtures/pqc-interop-vectors.json';
import {
...
// ============================================================
// CROSS-RUNTIME INTEROP — the direction laptop → worker.
//
// The fixture is generated by Elle's electron/native/providers/pqc-hybrid.cjs
// (Node/Electron) and committed here. Decapsulating it in this runtime must
// reproduce the same shared secret. The mirror of this test lives in the Elle
// repo and verifies worker → laptop from a fixture generated here.
//
// This is what the PQC design doc requires BEFORE any lane-derivation
// cutover: without it, a drift in either implementation would not surface
// until the sandbox bridge went silently deaf in production.
// ============================================================
describe('cross-runtime interop — laptop-generated vectors', () => {
  const fixture = laptopVectors as { origin: string; vectors: PqcVector[] };

  it('decapsulates Elle laptop-generated vectors to the identical shared secret', async () => {
    expect(fixture.origin).toMatch(/Elle laptop/);
    expect(fixture.vectors.length).toBeGreaterThanOrEqual(2);
    for (const v of fixture.vectors) {
      const { pk, sk, ct, sharedSecret } = importVector(v);
      const out = await pqcHybridDecaps(sk, pk, ct);
      expect(same(out, sharedSecret), `profile "${v.profile}" mismatched`).toBe(true);
    }
  });

  it('covers both profiles, so the hand-rolled QC-MDPC leg is exercised too', () => {
    expect(fixture.vectors.map((v) => v.profile).sort()).toEqual(['experimental', 'vetted']);
  });

  it('does not pass vacuously — a corrupted vector must not yield the secret', async () => {
    const { pk, sk, ct, sharedSecret } = importVector(fixture.vectors[0]);
    ct.mlkem[0] ^= 1;
    const out = await pqcHybridDecaps(sk, pk, ct);
    expect(same(out, sharedSecret)).toBe(false);
  });

  it('serializes vectors round-trip (the format both repos read and write)', async () => {
    const { publicKey, secretKey } = pqcHybridKeygen('experimental');
    const { ciphertext, sharedSecret } = await pqcHybridEncaps(publicKey);
    const v = exportVector(publicKey, secretKey, ciphertext, sharedSecret);
    const back = importVector(v);
    expect(same(await pqcHybridDecaps(back.sk, back.pk, back.ct), sharedSecret)).toBe(true);
  });

  it('hex helpers round-trip exactly', () => {
