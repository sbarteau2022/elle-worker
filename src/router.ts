...
grant_fit_analyses(id,org_id,opportunity_id,fit_index,confidence_interval,sample_size,created_at), grant_proposal_analyses(...), grant_development_sessions(...), grant_reasoning_log(id,subject_id,subject_type,conclusion,...), grant_necaif_evaluations(id,opportunity_id,revenue_mechanism,narrative_capture_history,editorial_conditions,mission_alignment,trust_of_affected_populations,documented_networks,...), grant_statistical_models(id,funder_id,feature_weights_json,methodology,...) — mostly empty, schema only, no ingest yet
grant_opportunities, grant_recipients, and grant_funder_990_overview are NOT in this database (they live in the GrantIntelligence repo's grant-worker) and are not queryable via read_sql — use the list_opportunities/get_990_overview actions on /api/elle-grants instead
`.trim();
...
const TOOL_LINES: Record<string, string> = {
  ...
  tax_business_create: `WRITE: tax_business_create(business_name,entity_type?:sole_prop|single_member_llc|multi_member_llc|s_corp|c_corp,state?:MO|KS|IL|IN,locality?:KC|STL,county?,county_tax_rate?,ein_last4?,industry_naics?) — register a new business for the signed-in tax client. county/county_tax_rate matter ONLY for Indiana (92 counties each set their own rate, 0.50%-3.38% — not looked up automatically; ask the user for their county's rate from Indiana DOR's table if state is IN). Idempotent per (user, business_name) — calling again with the same name resumes it rather than duplicating. Auto-files an onboarding review and (for supported entity types) arms a recurring quarterly-deadline reminder.`,
  ...
  edu_enroll: `edu_enroll(course?) — WRITE: enroll THIS user in a CustomCourseBuilder course. course ∈ {ai-engineer-stack (default; 12-month/5-track program mapped over external credentials), ai-engineer-curriculum (first-party, ours-to-teach; descend/build/re-ascend; currently covers the foundation tier as later tiers are authored)}. One enrollment per user; state is theirs alone.`,
  ...
  validate_kappa: `validate_kappa(limit?) — run the pre-registered κ validation over accumulated bending_trace rows (docs/MEMORY_KERNEL_SPEC.md §10). Scores the κ-trajectory signals (velocity_peak, reserve) against the independent lexical label (settled_open) as ROC-AUC and reports KILL / BUILD / INSUFFICIENT per kill-test against thresholds fixed in code (VELOCITY_BOUNDARY ≥ 0.70, RESERVE_CONSOLIDATION ≥ 0.65). Read-only: it NEVER writes the seam — Invariant Q3 keeps that a manual commit — and it flags that BUILD here clears only an internal proxy, necessary but not sufficient for the human-rater Gate 2. The master KAPPA_VALIDATED gate is reported un-validatable until a κ(T,t) definition exists (Gate 0). Use to see whether the gated memory layer has earned its flip yet.`,
...
const TOOL_TREE: { category: string; tools: string[] }[] = [
  ...
  { category: 'Provenance & self-audit', tools: ['provenance', 'constraint_analyzer', 'fork_replay', 'metabolism', 'validate_kappa'] },
...
export async function runTool(
  ...
  try {
    switch (name) {
      ...
      case 'validate_kappa': {
        // The pre-registered κ validation run (docs/MEMORY_KERNEL_SPEC.md §10).
        // Reads bending_trace, computes the kill-test AUCs, reports a verdict.
        // NEVER writes the seam — that stays a manual commit (Invariant Q3).
        const limit = Math.max(1, Math.min(50000, Number(a.limit) || 20000));
        const r = await env.DB.prepare(
          `SELECT settled_open, reserve, velocity_peak FROM bending_trace
           ORDER BY created_at DESC LIMIT ?`
        ).bind(limit).all().catch(() => ({ results: [] as unknown[] }));
        const rows = (r.results as unknown as KappaTraceRow[]) || [];
        return clip(JSON.stringify(runValidateKappa(rows)));
      }
      case 'pami':
